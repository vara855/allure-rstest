...
import { flag, getAssertionPrototype, getExpectStatic } from './chai.js';
import { getCurrentTask } from './currentTask.js';
import { markAsMatcherMessage } from './matcherMessages.js';

type ChaiUtils = { flag: typeof flag };
type Assertion = Record<PropertyKey, unknown>;
...
const ALLURE_RSTEST_EXPECT_PATCHED = Symbol.for('allure.rstest.expectPatched');
const ALLURE_RSTEST_EXPECT_EXTEND_PATCHED = Symbol.for('allure.rstest.expectExtendPatched');
const ALLURE_RSTEST_EXPECT_WRAPPED = Symbol.for('allure.rstest.expectWrapped');

const MAX_VALUE_LENGTH = 160;
const MAX_VALUE_DEPTH = 2;
const ACTIVE_MATCHER_STEP_FLAG = 'allure-rstest-active-matcher-step';
const JEST_ASYMMETRIC_MATCHER = Symbol.for('jest.asymmetricMatcher');
// rstest marks Chai assertions created by its expect() through withTest() with this flag;
// we only read it and let rstest remain the owner of the flag.
const RSTEST_TEST_FLAG = 'vitest-test';
...
const isWrapped = (value: unknown) =>
  ...
  Boolean((value as unknown as Record<PropertyKey, unknown>)[ALLURE_RSTEST_EXPECT_WRAPPED]);

const isAsymmetricMatcher = (value: unknown): value is AsymmetricMatcher =>
  Boolean(
    value &&
      typeof value === 'object' &&
      (value as AsymmetricMatcher).$$typeof === JEST_ASYMMETRIC_MATCHER,
  );

const markWrapped = <T extends AnyFunction>(value: T): T => {
  Object.defineProperty(value, ALLURE_RSTEST_EXPECT_WRAPPED, {
    value: true,
  });
...
const getCurrentRstestTask = (): RstestTask | undefined => {
  const task = getCurrentTask() as RstestTask | undefined;

  return task?.meta ? task : undefined;
};

// In concurrent tests, rstest's global current test can be reset after an awaited gap,
// so expect() may skip withTest() even though the assertion is still running inside a known task.
// Keep that recovery narrow to vitest/jest-style matcher names so raw Chai methods like equal()
// remain ignored.
const canRecoverRstestTaskFromAsyncContext = (assertionName: string) => assertionName.startsWith('to');

const getRstestTask = (
  utils: ChaiUtils,
  assertion: Assertion,
  assertionName: string,
): RstestTask | undefined => {
  const flaggedTask = utils.flag(assertion, RSTEST_TEST_FLAG) as RstestTask | undefined;
  const currentTask = getCurrentRstestTask();

  if (!flaggedTask?.meta) {
    return canRecoverRstestTaskFromAsyncContext(assertionName) ? currentTask : undefined;
  }

  return currentTask ?? flaggedTask;
};

const countErrors = (task: RstestTask | undefined) => task?.result?.errors?.length ?? 0;

const getNewSoftAssertionError = (task: RstestTask | undefined, beforeErrors: number) => {
...
const runWithNestedMatcherStepsSuppressed = <T>(
  utils: ChaiUtils,
  assertion: Assertion,
  body: () => T,
): T => {
...
const wrapAssertionMethod = (
  ...
  const wrapped = markWrapped(function (this: Assertion, ...args: unknown[]) {
    ...
    const task = getRstestTask(utils, this, assertionName);
...
const instrumentWithTest = (prototype: AssertionPrototype) => {
  ...
  const wrapped = markWrapped(function (this: Assertion, task?: unknown, ...args: unknown[]) {
    return original.call(this, getCurrentRstestTask() ?? task, ...args);
  });
...
const instrumentExistingAssertions = (utils: ChaiUtils, prototype: AssertionPrototype) => {
  if (prototype[ALLURE_RSTEST_EXPECT_PATCHED]) {
    return;
  }

  Object.defineProperty(prototype, ALLURE_RSTEST_EXPECT_PATCHED, {
    value: true,
  });
...
const instrumentExpectExtend = (utils: ChaiUtils, prototype: AssertionPrototype) => {
  const expectStatic = getExpectStatic();

  if (expectStatic[ALLURE_RSTEST_EXPECT_EXTEND_PATCHED]) {
    return;
  }

  Object.defineProperty(expectStatic, ALLURE_RSTEST_EXPECT_EXTEND_PATCHED, {
    value: true,
  });
...
export const registerAllureRstestExpect = (): void => {
  const prototype = getAssertionPrototype();
  const utils: ChaiUtils = { flag };

  instrumentWithTest(prototype);
  instrumentExistingAssertions(utils, prototype);
  instrumentExpectExtend(utils, prototype);
};